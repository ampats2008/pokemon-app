...
const PkmnCard = ({obj, setLoadedCardsCount}:Props, forwardRef:any) => {
    ...
    const getPkmnObj = async () => {
        ...
        let descriptions = await speciesRes.flavor_text_entries.filter((entryObj:any) => entryObj.language.name === 'en').sort((a:any, b:any) => {
        ...
        let description = await descriptions.sort().find( (entryObj:any) => 
        ...
        let genera = await speciesRes.genera.filter((entryObj:any) => entryObj.language.name.toLowerCase() === 'en');
